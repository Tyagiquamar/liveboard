...
- **2026-08-26 14:40** — Portfolio-sprint finish (new agent, full re-verification):
  - Re-verified every WIP claim from scratch on this machine: server tests **7/7**, typechecks clean (server+web), `next build` succeeds, seed idempotent, two-window E2E **7/7** with fresh screenshots regenerated from the running stack (web :27441, api :27442, mongo :27443).
  - **Demo mode shipped** (`web/src/lib/demo.ts`): build-time `NEXT_PUBLIC_DEMO=1` swaps the fetch layer for an in-memory seeded backend (same Acme data/shape as the server) and stubs the socket; mutations emit real event objects through the same client pipeline, state persists to localStorage and syncs live across tabs via storage events. Clearly labelled with an always-visible banner; login page explains the mode. Verified by `web/scripts/demo-check.mjs`: **11/11 locally, 11/11 against the deployed URL**.
  - **Deployed web to Vercel** (project `liveboard`, demo mode): https://liveboard-red.vercel.app — verified with the script above (28 seeded cards render, drag works, cross-tab sync works, reload persists, zero console errors).
  - `docs/DEPLOY.md` written: exact runbook to restore the full hosted stack once a long-running host exists (Railway trial expired — backend hosting is the only missing piece).
  - README: Live Demo section, demo-mode caveats, actual test results, deployment status.
  - Final gates re-run after all changes: server tests 7/7, typechecks clean, non-demo `next build` succeeds, `docker compose config` valid.
